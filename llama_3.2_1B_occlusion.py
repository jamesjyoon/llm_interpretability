...
def _select_best_variant(metrics: Dict[str, Dict[str, float]]) -> Tuple[str, Dict[str, float]]:
    ...
    Falls back to raw metrics when no calibrated variant improves F1.
    """
...
def run_occlusion(model, tokenizer, texts, device, config, formatter, prefix, calibrator: Optional[Callable[[np.ndarray], np.ndarray]] = None):
    """
...
def run_experiment(args: argparse.Namespace) -> None:
    ...
    if config.run_occlusion:
        run_occlusion(model, tokenizer, eval_texts, device, config, formatter, "zero_shot", zero_shot_calibrator)
    ...
    if args.finetune:
        ...
        if config.run_occlusion:
            run_occlusion(classification_model, tokenizer, eval_texts, device, config, formatter, "fine_tuned", fine_tuned_calibrator)

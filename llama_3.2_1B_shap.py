...
try:
    ...
    raise SystemExit("Install shap: `pip install shap`") from exc
...
def run_shap(model, tokenizer, texts, device, config, formatter, prefix, calibrator=None):
    """Run SHAP text explanations and save results."""
    ...
    def predict_fn(text_list):
        ...
        model.eval()
        for i in range(0, len(prompts), config.shap_batch_size):
            batch = prompts[i:i + config.shap_batch_size]
            inputs = tokenizer(batch, padding=True, truncation=True,
    ...
    # Create SHAP explainer using Partition explainer for text
    ...
    for i, text in enumerate(samples):
        ...
        values = shap_values[i, :, pred_idx].values
        data = shap_values[i, :, pred_idx].data
        
        token_weights = [(str(tok), float(val)) for tok, val in zip(data, values) if tok.strip()]
        ...
        shap_outputs.append({
            ...
            "token_weights": token_weights[:config.interpretability_example_count * 2],
        })
    ...
    # Plot SHAP
    _plot_shap(shap_values, shap_outputs, config.output_dir, prefix)
    
    return shap_outputs


def _plot_shap(shap_values, shap_outputs, output_dir, prefix):
    ...
    for ax, item in zip(axes, shap_outputs):
        weights = item['token_weights'][:15]  # Top 15 features
        if not weights:
            continue
    ...
    plt.tight_layout()
    path = os.path.join(output_dir, f"{prefix}_shap_plot.png")
    plt.savefig(path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"Saved {path}")
    
    # Also save SHAP's native text plot if available
    try:
        shap.plots.text(shap_values[0], display=False)
        plt.savefig(os.path.join(output_dir, f"{prefix}_shap_text_plot.png"), dpi=150, bbox_inches='tight')
        plt.close()
    except Exception:
        pass
...
def run_experiment(args: argparse.Namespace) -> None:
    ...
    # Run SHAP for zero-shot
    if config.run_shap:
        zs_calibrator = _temperature_calibrator(zs_calib.get("temperature")) if zs_calib else None
        run_shap(zs_model, tokenizer, eval_texts, device, config, formatter, "zero_shot", zs_calibrator)
    ...
    if config.finetune:
        ...
        # Run SHAP for fine-tuned
        if config.run_shap:
            ft_calibrator = _temperature_calibrator(ft_calib.get("temperature")) if ft_calib else None
            run_shap(ft_model, tokenizer, eval_texts, device, config, formatter, "fine_tuned", ft_calibrator)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--no-shap", dest="run_shap", action="store_false")

...
def _load_label_token_map(tokenizer) -> LabelTokenMap:
    ...
    for label in (0, 1):
        token_ids = tokenizer(
            str(label), add_special_tokens=False, return_attention_mask=False
        )["input_ids"]
        if not token_ids:
            raise ValueError(f"Tokenizer could not encode label {label}.")
        if len(token_ids) > 1:
            raise ValueError(
                "Label token is split into multiple ids. Provide labels that map to a single token."
            )
        label_token_map[label] = token_ids[0]

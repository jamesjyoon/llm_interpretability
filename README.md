...
| **F7.2 ROAR (Faithfulness)** | **Literature / Computational** | *Remove And Retrain (ROAR)* requires retraining the LLM dozens of times to measure accuracy drops when top features are removed. Due to the high computational cost of retraining Llama-1B, we utilize literature baseline scores (LIME 0.58, SHAP 0.60) rather than running this benchmark dynamically. |

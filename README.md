...
| F7.2 ROAR (Faithfulness)        | Literature / Computational | Remove And Retrain (ROAR) requires retraining the LLM dozens of times to measure accuracy drops when top features are removed. Due to the high computational cost of retraining Llama-1B, literature baseline scores (LIME ~0.5, SHAP ~0.0) are used rather than running these benchmarks dynamically. |
| F2.2 Graphical Integrity        | Qualitative                | This measures the visual clarity of the output (for example, bar charts). As this is a subjective HCI metric, it cannot be computed by a script.                                                                                                                                                       |
| F6.2 Surrogate Agreement (SHAP) | Computational              | SHAP (0.6). Calculating exact fidelity for KernelSHAP requires convergence (thousands of samples), which is too slow for this script, so a conservative estimate is used based on the low nsamples setting in this experiment.                                                                         |

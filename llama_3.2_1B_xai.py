...
def parse_arguments():
    ...
    # Defaulting to 500 samples for training to ensure completion within time limits
    parser.add_argument("--train-size", type=int, default=500, help="Number of samples for fine-tuning. Set to -1 for full dataset.")
...
def calculate_jaccard(list1, list2):
    """Calculates Jaccard Index for stability/identity."""
    s1 = set(list1)
    s2 = set(list2)
...
def compute_xai_properties(model, tokenizer, eval_data, sample_size, predict_fn, phase_name=""):
    ...
    results["LIME"]["F4.1_Contrastivity_Level"] = round(np.mean(metrics["LIME"]["contrast"]) * 5, 2)
    results["kernelSHAP"]["F4.1_Contrastivity_Level"] = round(np.mean(metrics["kernelSHAP"]["contrast"]) * 5, 2)
...
def main():
    ...
    if args.finetune:
        ...
        if args.train_size > 0 and args.train_size < len(full_train_ds):
            ...
            print(f"Using FULL training dataset ({len(full_train_ds)} samples). WARNING: This may take hours.")
        ...
        # Save Performance Comparison immediately
